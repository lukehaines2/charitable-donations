...
export const CharityList = props => {
  ...
  const onClick = (e) => {
    ...
    // (We're using isLoading in parent to block concurrent calls)
    if (!isLoading && chosen !== selectedCharity) handleCharityClick(chosen);
  };

  // NOTE: Bit of a hack for handling strings & numbers in state of selectedCharity - Ideally would never have alternating use of strings and numbers across our components
